...
- **D-13:** Создание учётки при первом запуске — тоже CLI-only (`create-admin`, одна механика с D-02). Веб-экрана setup нет.
...
- **D-14:** В ежедневный бэкап-джоб входит `PRAGMA integrity_check` копии — дешёвая страховка от битых бэкапов. Полный авт restore-тест не делаем; восстановление репетируется вручную при сдаче фазы (success criterion).
...
- **D-15:** Канонический remote — корпоративный GitLab (приватный): код + `.planning/`, git pull деплоя тянется оттуда. Публичный GitHub — отдельное зеркало с чистым кодом без `.planning/` и рабочих данных, когда-нибудь позже; рабочее на GitHub не утекает.

### Данные и схема (грилинг)
- **D-16:** Инвентарные номера — только ручной ввод, автогенератора нет. Номера присваивают 1С-базы бухгалтерии и ИТ (формат вида `ИБ-0000146`); в приложении вписываешь присвоенный номер как есть.
- **D-17:** UNIQUE на нормализованный инвентарник: дублей в 1С не бывает, сработавший констрейнт = опечатка ввода, её надо ловить. UNIQUE на серийники — из research (подтверждено).
- **D-18:** Отделы — справочник: таблица `departments`, начальный список заводит пользователь. CRUD справочника — UI Фазы 2.
...
- Форма и имя CLI-команды сброса/создания админа
- Планировщик бэкапа: cron внутри контейнера vs host-cron — на выбор планировщика
- Механика нормализации номеров (регистр, пробелы, гомоглифы) — по PITFALLS.md
