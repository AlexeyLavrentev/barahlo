...
```bash
git clone https://github.com/AlexeyLavrentev/barahlo.git && cd barahlo   # или git pull, если уже склонировано
...
Финальный чек-лист приёмки фазы (успех-критерии 4 и 5). Шаги (2)–(8) выполняет оператор на внутреннем сервере и закрывает на приёмке фазы (end-of-phase); выполненные шаги отмечаются прямо в этом README. Ни один шаг не ослабляет периметр.

1. ☑ Код и `.planning/` запушены в канонический remote: `git push -u origin main` — выполнено 2026-09-01 (приватный GitHub-репозиторий, см. «Требования к серверу»).
2. ☐ На сервере: склонировать из origin (`git clone https://github.com/AlexeyLavrentev/barahlo.git`; или `git pull`, если уже склонировано), затем `cp .env.example .env`, `openssl rand -base64 32` → `AUTH_SECRET=...` в `.env` (прод-пути `DATABASE_PATH=/app/data/app.db`, `DATA_DIR=/app/data`), `mkdir -p data && sudo chown -R 1000:1000 data`
3. ☐ `bash scripts/deploy.sh` — первый прогон собирает образ, применяет миграции, поднимает контейнер и ставит cron-строку ночного бэкапа
4. ☐ На сервере создать учётку: `docker compose exec -T app node scripts/create-admin.mjs`
5. ☐ Из браузера рабочей станции: открыть `http://<server>:3000` → редирект на `/login` → войти → каркас-экран; перезапустить браузер — сессия жива (cookie на 30 дней)
6. ☐ Curl-матрица периметра на сервере — те же команды из «Проверки периметра»: без сессии `/` → 307, `/api/health` → 307, `/login` → 200
7. ☐ `crontab -l` содержит ровно одну строку с `scripts/backup.mjs`; на следующее утро проверить `./data/backups/<вчерашний день>/`
8. ☐ Серверная репетиция восстановления по процедуре «Восстановление (пошагово)» — только с остановленной стопкой (шаг 1 процедуры не опционален); хотя бы один раз в первый месяц эксплуатации
9. ☐ Отметить выполненные шаги прямо в этом списке

Локальная половина приёмки уже закрыта: репетиция восстановления выполнена 2026-09-01 (см. «Репетиция восстановления — выполнена»), переживание рестарта доказано.
